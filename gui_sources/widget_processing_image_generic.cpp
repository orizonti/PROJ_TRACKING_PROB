...
void WidgetProcessingImage::slotDisplayImage()
{
   ...
   if(Thinning.isOpen()) 
   {
   auto TickPeriods = ImageSources[0]->getTickPeriod();
   strDisplayData = QString("ПЕРИОД: %1  ОБРАБОТКА: %2").arg(1/TickPeriods.first,5,'f',2).arg(TickPeriods.second,5,'f',2);
   ...
   QPainter paint;
            ...
            for(auto& rect: RectsImage){paint.setPen(*pen_it); 
                                        ...
                                        paint.drawPoint(ui->labelImageDisplay->PosPressed.first*DisplayImage.width(), 
                                                        ui->labelImageDisplay->PosPressed.second*DisplayImage.height());
...
void WidgetProcessingImage::linkToModule(std::shared_ptr<SourceImageDisplayInterface> Source)
{
   NumberActiveChannel++;
